...
export default function AIGeneratePage(){
  ...
  function buildPrintableWorksheetHTML(worksheet) {
    ...
    let body = ''
    body += `<h1>${escapeHtml(worksheet.title || 'Worksheet')}</h1>`
    body += `<div class="meta">Subject: ${escapeHtml(worksheet.subject || '')} · Grade: ${escapeHtml(worksheet.grade || '')}</div>`
    if (worksheet.description) {
      body += `<div>${escapeHtml(worksheet.description)}</div>`
    }
      ...
      questions.forEach((q, qi) => {
        ...
        if (q.type === 'multiple_choice' && Array.isArray(q.options)) {
          ...
          q.options.forEach((opt, oi) => {
            body += `<div>${String.fromCharCode(65 + oi)}. ${escapeHtml(opt)}</div>`
          })
          body += `</div>`
        }
        body += `</div>`
